...
//生成证书
/*func createGmSm2Cert(key core.Key, req *csr.CertificateRequest, priv crypto.Signer) (cert []byte, err error) {
	log.Infof("xxx xxx in gmca.go  createGmSm2Cert...key :%T", key)

	csrPEM, err := generate(priv, req, key)
	if err != nil {
		log.Infof("****** create csr error:%s ******\n", err)
	}
	log.Infof("****** create gm csr completed! ******\n")
...
func generate(priv crypto.Signer, req *csr.CertificateRequest) (csr []byte, err error) {
	...
	if err != nil {
		log.Debugf("internal/.../fabric-ca/lib/gmUtil.go: failed to generate a CSR: %v", err)
		return
	}
	block := pem.Block{
